...
class TravelSearchForm extends Component {
  ...
  onChangeDestination = (event, { newValue, method }) => {
    ...
    if(newValue.substring("(")){
      var destination = newValue.substring(newValue.length - 4, newValue.length - 1);
      this.setState({
  ...
  render() {
    const { value, valueDest, suggestions, suggestionsDestination } = this.state;
    const inputProps = {
